...
class BookChunked:
    ...
    def getChapterContent(self):
        ...
        print("ch: ", self.base)
        print("cp: ", self.chapterPaths)

        for chapterPath in self.chapterPaths:
            chapter.append(chapterText(os.path.join(self.base, chapterPath, chapterPath+'.json')))
        return chapter
    
    def getAllChunks(self):
        chapterChunks = []
        for chapterPath in self.chapterPaths:
            with open(os.path.join(self.base, chapterPath, chapterPath+'.json'), 'r') as f:
